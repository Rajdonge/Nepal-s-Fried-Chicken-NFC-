...
class Cart(models.Model):
    ...
    def __str__(self):
        if self.user:
            ...
            customer = f"Guest ({self.guest_id[:8] if self.guest_id else 'Unknown'})"
        else:
...
class Address(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Convert address_type to lowercase
        if self.address_type:
            self.address_type = self.address_type.lower()
        
        # Auto-fetch location details if location is provided and coordinates are missing
        if self.location and (self.latitude is None or self.longitude is None):
            print(f"Fetching coordinates for location: {self.location}")
            from api.utils import get_location_details
            details = get_location_details(self.location)
            if details:
                ...
                if details['province']:
                    self.province = details['province']
                if details['district']:
                    self.district = details['district']
                if details['postal_code']:
                    self.postal_code = details['postal_code']
                print(f"Saved coordinates: {self.latitude}, {self.longitude}")
            else:
                print(f"Failed to get coordinates for: {self.location}")
...
class Invoice(models.Model):
    ...
    billing_full_name = models.CharField(max_length=200,null=True,  blank=True)
    billing_phone = models.CharField(max_length=15,null=True,  blank=True)
    billing_email = models.EmailField(null=True, blank=True)
    billing_address = models.TextField(null=True, blank=True)
    billing_city = models.CharField(null=True, max_length=100, blank=True)
    billing_province = models.CharField(null=True, max_length=50, blank=True)
